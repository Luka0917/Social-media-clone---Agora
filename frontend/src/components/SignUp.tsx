...
export default function SignUp({ toggleAuth }: SignInProps){
    ...
    return(
        <div className="slide-in-bottom-animation w-full max-w-sm">
            <h1 className={`${theme === 'light' ? 'text-[#0e141f]' : 'text-[#eef2f7]'} font text-4xl font-bold`}>Sign up</h1>
            <p className={`${theme === 'light' ? 'text-[#747d8b]' : 'text-[#9099a6]'} text-sm`}>A warmer place to share your world.</p>
            <form onSubmit={SignUpForm} className="mt-8">
                <div className="flex flex-col">
                    <label htmlFor={usernameId} className={`${theme === 'light' ? 'text-[#0e141f]' : 'text-[#eef2f7]'} text-sm font-medium`}>Username</label>
                    <input type="text" name="username" autoComplete="username" id={usernameId} className={`${theme === 'light' ? 'border-[#dae0e8] ring-[#1e77ed] placeholder:text-[#9098a3]' : 'border-[#2b2e35] ring-[#4288ed] placeholder:text-[#7b848f] text-[#eef2f7]'} rounded-full h-9.5 shadow-sm text-sm border pl-3 outline-none focus:ring-1`} />
                </div>
                ...
                <button className={`${theme === 'light' ? 'text-[#eef2f7] bg-[#1874ed] hover:opacity-80' : 'text-[#0e141f] bg-[#4490fe] hover:opacity-80'} w-full mt-3 border rounded-full py-2 font-medium cursor-pointer transition-all duration-200`}>Sign up</button>
            </form>
            <p className={`${theme === 'light' ? 'text-[#7a828f]' : 'text-[#8c95a2]'} text-center mt-5 text-sm`}>
                Already have an account?
                <span onClick={toggleAuth} className={`${theme === 'light' ? 'text-[#1874ed]' : 'text-[#4490fe]'} ml-1 font-medium hover:underline cursor-pointer`}>Sign in</span>
            </p>
        </div>
    )
}